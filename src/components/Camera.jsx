...
export default function Camera({ side, image, onCapture, onRetake, compact }) {
  ...
  const start = useCallback(async () => {
    try {
      ...
      // Camera access denied — handled by showing upload fallback
    }
  }, []);
  ...
  if (image) {
    return (
      <div className="card" style={{ flex: "1 1 140px", minWidth: 120, textAlign: "center", position: "relative", padding: 8 }}>
        ...
        <img src={image} alt={side} style={{ width: "100%", maxHeight: mH, borderRadius: 8, objectFit: "contain", background: "#000", marginTop: 12 }} />
        <button className="btn-g" onClick={onRetake}>\u21bb</button>
      </div>
    );
  }
      ...
      {live ? (
        <>
          ...
          <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 6 }}>
            <button onClick={snap} style={{ width: 44, height: 44, borderRadius: "50%", border: "3px solid var(--acc)", background: "transparent", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: "linear-gradient(135deg,var(--acc),var(--acc2))" }} />
            </button>
            <button className="btn-g" onClick={stop}>\u2715</button>
          </div>
        </>
      ) : (
        <div style={{ padding: compact ? "10px 0" : "16px 0" }}>
          ...
          <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
            <button className="btn-a" onClick={start}>\ud83d\udcf7</button>
