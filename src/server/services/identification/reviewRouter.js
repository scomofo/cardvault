...
export function routeReview(result, item) {
  ...
  const estimatedValue = Number(item.market_price || item.suggested_listing_price || 0);
  return {
    queue: estimatedValue >= 100 ? "review_high_value" : "review",
    severity: estimatedValue >= 100 ? "high" : "medium",
  };
}
