...
- `src/hooks/` — `useScanWorkflow.js` (single-card scan), `useBatchWorkflow.js` (resumable scan intake), `useFeeModels.js`.
...
- **Host/Origin enforcement** (`src/server/auth.js` `enforceTrustedHostAndOrigin`, mounted on `/api` in `server.js`): without `PROXY_TOKEN`, a request whose `Host` is not in `getTrustedDevHosts()` (localhost, loopback, the machine's interface addresses, `DEV_HOSTNAME`) gets 403, and a non-GET request with a foreign `Origin` gets 403. Integration tests hit `127.0.0.1` so they pass; a test that sets a custom `Host` must expect 403. Marketplace publish/revise/end/sync/crosspost/handoff and `POST /api/automation/shipping/:orderId` sit behind `requireProtectedConfigWrite` like the settings routes.
- **Outbound URL guard** (`src/server/outboundUrlGuard.js`): partner/shipping URLs are rejected for any loopback (all of 127/8, IPv4-mapped IPv6), private, link-local, CGNAT, or unresolvable host, and `fetchPublic` drops `Authorization`/API-key headers on a cross-origin redirect. Tests that stand up a local partner set `CARDVAULT_ALLOW_LOCAL_OUTBOUND_URLS=1` (the test helper does).
- **Offline-first sync**: client state lives in localStorage (`cv8_*` keys) + React state via `DataContext`. Setters schedule a **500ms-debounced** diff-by-id sync (`src/lib/sync/syncEngine.js`) against a boot-time snapshot; there is **no flush/await mechanism**. `POST /api/items` and `POST /api/listings` are **idempotent upserts by id** so redundant creates (engine retries, direct create + engine create) are harmless — the listings upsert path deliberately never touches server-managed columns (`publish_status`, `publish_error`, `external_listing_id`, `last_sync_at`). Generic listing PUT also preserves status, publish and sale fields when a channel exists; sales/order routes and marketplace services own those transitions. Do not recompute channel aggregate state from generic PUT: a manual sale can be authoritative while its channel still says draft/active.
- **diffById**: a changed `updatedAt` always marks a record changed; a matching (or absent) `updatedAt` is *not* treated as proof of no change — it always falls through to a field-by-field comparison, since local edits never bump `updatedAt` themselves.
...
- **Batch durability**: `storage.js` uses IndexedDB version 2 (`images` plus `batch_sessions`). Scan and Tools intake use separate `scan` / `tools` sessions on the current browser/device. Persist successful removals before clearing their queue entries. Failed/unnamed/review entries stay queued. Reuse intake IDs for item/image retries; require an explicit inspected condition. AI confidence labels are not calibrated probabilities, and AI pricing evidence is unverified.
- **Publish retries**: live eBay publication persists `publishing` before its create request. Confirmed real IDs are reused; ambiguous failures become `publish_unknown`. The API blocks retry until the user checks Seller Hub and passes `confirmNotPublished: true`; an active 120-second claim cannot be bypassed. Disconnecting cannot erase uncertainty. A successful timeout does not imply failure remotely.
- **Draft review**: `DraftReview` edits content and photo links through protected `POST /api/listings/:id/review`, which validates in a transaction and preserves unrelated item data. Missing offline drafts recover only after a confirmed 404, reusing item/listing IDs. Server-only image previews use authenticated `imagesAPI.read` blobs. `listingContent.js` is shared by client and server and must stay in Electron `build.files`. Live eBay preflight validates basic content, inspected raw condition and actual stored photos. Raw condition never implies a professional grade. A failed photo upload is `EBAY_PREPARATION_FAILED` and safely returns to draft before any listing-create call; failures after create remain uncertain.
- **Checked eBay fixed-price publication**: `ebayListingCheckService.js` stores account-bound selling defaults and single-use 15-minute checks in SQLite. Checking loads actual policies, uploads both photos, and calls `VerifyAddFixedPriceItem` without creating a listing. Publication requires the exact receipt ID, explicit approval and environment; it reloads policies and checks content/photo-byte/account fingerprints immediately before sending the stored XML. Never accept checked XML or successful verification from a client. Auction behavior remains separate. Unknown fixed-price attempts return to editable draft only through explicit protected recovery after Seller Hub review and the existing active-claim guard. Preserve concurrent sold/ended/confirmed state when a publish guard fails. See `docs/eBay-Checked-Drafts.md`.
- **Shipping**: a rate quote is `pending`, a provider-confirmed label is `label_purchased`, and neither is dispatched. Never synthesize tracking, label URLs, charges or dispatch timestamps. An uncertain/repeated purchase needs provider-history review (`retry` plus `confirmNoExistingLabel`), with an active purchasing claim protected for 120 seconds. `POST /api/orders/:id/dispatch` requires paid status and `confirmed: true`; entering tracking alone is not dispatch. Existing legacy shipment rows are not automatically rewritten.
...
- **Seller home**: `DashboardView` opens `SellerHome`; analytics remain collapsed below. `sellerWorkflow.js` separates reviewed drafts, confirmed live listings, publication exceptions and paid shipping work using existing lifecycle evidence. Bulk eBay draft preparation keeps edits and retry IDs in device storage, checkpoints IDs before requests, and uses a shared in-tab save lock across remounts. Leaving preparation pauses subsequent cards; retry recovers the same draft ID. Condition and price remain explicit review fields. `scan_mode` / `batch` focus waits for `batchReady` before opening capture or resuming a saved queue.
