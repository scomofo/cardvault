...
- `src/hooks/` — `useScanWorkflow.js` (single-card scan), `useBatchDraft.js` (shared batch-to-draft queue), `useFeeModels.js`.
...
- **Batch Sell v1**: `BatchSellView` replaces the two batch entry points. `batchDraftStore.js` serializes persisted edits across remounts; `transactSellingBatch` atomically migrates legacy `scan`/`tools` sessions and photos into the `selling` session with revision conflict checks. Photos remain separate from lightweight queue metadata. `batchDraft.js` holds testable readiness/preview/selection logic; `batchDraftSave.js` saves only selected ready entries and preserves failures. Identity/condition require review; prices are unverified estimates. The create-only `/api/listings/draft` route commits new item + draft atomically, reuses stable retry IDs, blocks sold/listed/conflicting items and requires real photos. It never publishes or overwrites existing inventory. See `docs/Batch-Sell-V1.md`.
...
- **Navigation contract** (`App.jsx`): views are `dashboard | sell | scan | cards | sales | dealer | tools | more | settings`; `handleNavigate` accepts a string or `{ view, toolsTab, focus }` (`toolsTab`: `batch|sets|grade|watch|trade`; `focus`: `{ type, id }` record deep-link held as `pendingFocus`, consumed by SalesFlow — tab switch + scroll/highlight via `sf-rec-<id>` element ids — and CatalogView — opens card detail). Queue/search→target mapping lives in `src/lib/search/searchNavigation.js`.
